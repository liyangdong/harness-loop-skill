...
Java project combining SDD (spec-first) and TDD (test-first) under harness-loop constraints.

> **Hybrid methodology:** this repo combines **SDD + TDD** (per Q2=Hybrid,
> Q2_sub=[SDD, TDD]). Spec-first then test-first: write the spec under
> `docs/specs/`, then drive implementation via `tests/`. SDD takes priority
> ordering (SDD > TDD > BDD > DDD > RDD) — when in conflict, the spec wins.
...
- scripts/check-promise.sh — detects <promise>DONE</promise> completion signal
